...
public class GoblinEnemyController : MonoBehaviour, IEnemy
{
    ...
    void Update()
    {
        ...
        if (isAlive || enemyStats.canJump || !attackController.isAttacking || !damageController.isKnockedBack)
        {
    ...
    {
        if (jumpController.isChargingJump || damageController.isKnockedBack || attackController.isAttacking || attackController.isChargingAttack || isIdling) return;
    ...
    {
        if (attackController.isAttacking || attackController.isChargingAttack) return;
