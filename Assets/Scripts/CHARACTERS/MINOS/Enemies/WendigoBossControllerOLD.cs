...
public class WendigoBossControllerOLD : MonoBehaviour, IEnemy
{
    [Header("Enemy Stats")]
    ...
    private void EnemyAttack()
    {
        ...
        if (Vector2.Distance(transform.position, playerTarget) <= attackRange && nextAttack <= 0 && !isKnockedBack && !isAttacking)
        {
    ...
    {
        enemyAnimator.SetTrigger("EnemyAttack1");
    ...
    {
        Instantiate(deathParticles, new Vector2(transform.position.x, transform.position.y - 1.75f), Quaternion.identity);
